...
def downsample(attn_res: torch.tensor, q_W: torch.tensor, q_H: torch.tensor):
    ...
    if q_W_prev > q_W:
        ...
        # dim_2 pooling
        attn_res = attn_res.transpose(-1, -2).reshape(B * n_heads, num_patches_H_2, num_patches_W,
                                                      num_patches_W)  # (B * n_heads, num_patches_H_2, num_patches_W, num_patches_W)
        attn_res = pool(attn_res)
        attn_res = attn_res.reshape(B * n_heads, num_patches_H_2, q_W).transpose(-1,
                                                                                 -2)  # (B * n_heads, q_W, num_patches_H_2)
    ...
    if q_H_prev > q_H:
        ...
        attn_res = attn_res.reshape(B * n_heads, q_W, num_patches_H,
                                    num_patches_H)  # (B * n_heads, q_W, num_patches_W, num_patches_H)
        attn_res = pool(attn_res)
        ...
        attn_res = attn_res.reshape(B * n_heads, q_W, num_patches_H,
                                    num_patches_H)  # (B * n_heads, q_W, num_patches, num_patches)
