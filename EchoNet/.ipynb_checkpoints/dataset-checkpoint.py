...
if __name__ == '__main__':
    path = 'G:\\EchoNet-Dynamic'
    echo = EchoNet(root=path)
    print(len(echo))
    # print(echo[10])
    image = echo[0]['image']
    print(image.shape)
    ...
    counts, bins, patches = plt.hist(vector, bins=10, alpha=0.6, color='g')
    for count, x in zip(counts, bins):
        # 在柱的中间位置显示计数
        plt.text(x + (bins[1] - bins[0]) / 2, count, str(int(count)), ha='center', va='bottom')
    plt.xlim(vector.min(), vector.max())

    # 重新设置横轴的刻度，显示最小值和最大值
    plt.xticks([vector.min(), vector.max()], labels=[f'Min: {vector.min():.2f}', f'Max: {vector.max():.2f}'])
    plt.ylim(0, max(counts) * 1.1)  # 为了让计数显示更清晰，加一点间距
