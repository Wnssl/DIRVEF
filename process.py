...
def get_preprocessed_tensor(dicom_file_path, fps=None, hr=None, orientation="Mayo"):
...
def show_video(video_path='1.avi'):
    ...
    while True:
        ret, frame = cap.read()
        if not ret:
            print("Reached end of video or canno                 t read frame.")
            break
...
def show_tensor(file_name):
    tensor = get_preprocessed_tensor(file_name)
    print(tensor.shape)

    data = tensor
    data = data.permute(0, 2, 1, 3, 4).numpy()[0][1]
...
def dicom2avi(dicom_file, input_dir='data', output_dir='video'):
    file_name = dicom_file.split('.')[0]

    dicom_file = os.path.join(input_dir, dicom_file)
    ...
    frames, channels, weight, height = tensor.shape
    cropSize = (224, 224)
    tensor = tensor.permute(0, 2, 3, 1)
    fourcc = cv2.VideoWriter_fourcc('M', 'J', 'P', 'G')
    video_filename = os.path.join(output_dir, file_name + '.avi')
    cropSize = (224, 224)
    out = cv2.VideoWriter(video_filename, fourcc, fps, cropSize, True)
    for i in tensor:
        output = (i[:, :, 1] * 255).clamp(0, 255).numpy()
    ...
    out.release()
    cv2.destroyAllWindows()
    print(f"{video_filename} finish convert")


if __name__ == '__main__':
    # please check if the path is right
    assert 1 == 0, "please update true file path and then delete this assert in process.py"
    src = "E:\\data\\"
    targets = "E:\\train_video\\"
    for file in os.listdir(src):
        print(file)
        dicom2avi(file, src, targets)